...
token;
 ...
 getByCategory(category:string,token):Observable<Data[]>{
      console.log("in search service");
       let headers = new Headers({ 'Authorization': 'Bearer ' + token });
      let options = new RequestOptions({ headers: headers });
     return this.http.get('http://172.17.120.57:9890/lims/getCategoryWiseBooks/'+category,options)
     .map((res:Response)  => res.json());
  }
   getCategories(token):Observable<Data[]>{
      console.log("in search service");
       ...
       let headers = new Headers({ 'Authorization': 'Bearer ' + token });
      let options = new RequestOptions({ headers: headers });
     return this.http.get('http://172.17.120.57:9890/lims/getCategories',options)
     .map((res:Response)  => res.json());
  }
   ...
   getAllBooks(token):Observable<Data[]>{
     ...
     let headers = new Headers({ 'Authorization': 'Bearer ' + token });
      let options = new RequestOptions({ headers: headers });
    return this.http.get('http://172.17.120.57:9890/lims/getAllBooks',options)
     .map((res:Response)  => res.json());
  }
      getAllBook():Observable<Data[]>{
      console.log('inside service getallbooks2');
