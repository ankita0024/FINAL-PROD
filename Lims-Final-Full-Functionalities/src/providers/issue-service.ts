...
export class IssueService {
  ...
  request(mid, isbn, token) {
    ...
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get('http://172.17.120.57:9890/lims/requestBook/' + mid + '/' + isbn, options)
      .map((res: Response) => res);
  }
  ...
  availabilty(isbn, token): Observable<data[]> {
    let headers = new Headers({ 'Authorization': 'Bearer ' + token });
    let options = new RequestOptions({ headers: headers });
    return this.http.get('http://172.17.120.57:9890/lims/availableBooks/' + isbn, options)
